...
export async function POST(req) {
  try {
    ...
    if (!res.ok) {
      return NextResponse.json({ message: data.msg || 'Failed to register' }, { status: res.status });
    }

    return NextResponse.json({ message: data.msg || 'User registered successfully' });
  } catch (err) {
...
export async function PATCH(req) {
    try {
        ...
        if (!res.ok) {
            return NextResponse.json({ message: data.msg || 'Upload failed' }, { status: res.status });
        }
