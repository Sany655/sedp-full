...
export async function PATCH(req) {
  try {
    ...
    const res = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/locations/${regionId}`, {
      method: 'PATCH',
      headers: {
        'Content-Type': 'application/json',
        Authorization: `Bearer ${token}`,
...
export async function DELETE(req) {
    ...
    try {
        ...
        const res = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/locations/${id}/delete`, {
            method: 'DELETE',
            headers: {
                Authorization: `Bearer ${token}`,
            },
