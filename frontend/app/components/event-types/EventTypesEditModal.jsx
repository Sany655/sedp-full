 ...
 *  - eventtype: object (existing item)
 */
const EventTypesEditModal = ({ isOpen, onClose, onSubmit, eventtype }) => {
  const [formData, setFormData] = useState({
    eventtype_name: "",
    eventtype_description: "",
  });

  // Populate form data when editing an existing eventtype
  useEffect(() => {
    if (eventtype) {
      setFormData({
        eventtype_name: eventtype.name ?? eventtype.eventtype_name ?? "",
        eventtype_description:
          eventtype.description ?? eventtype.eventtype_description ?? "",
      });
    } else {
      // if no eventtype, clear form
      setFormData({
        eventtype_name: "",
        eventtype_description: "",
      });
  ...
  const handleSubmit = (e) => {
    ...
    if (!eventtype) {
      console.error("No eventtype provided for edit");
      return;
    }

    // Get id from eventtype prop, support both SQL and Mongo style
    const id = eventtype.id ?? eventtype._id;
    if (!id) {
      console.error("Event type id missing");
      return;
    }
    ...
    const payload = {
      ...
      name: formData.eventtype_name.trim(),
      description: formData.eventtype_description.trim(),
      // keep legacy keys if you used them elsewhere
      eventtype_name: formData.eventtype_name.trim(),
      eventtype_description: formData.eventtype_description.trim(),
    };
  ...
  const handleClose = () => {
    // Reset form to original eventtype values (if any)
    if (eventtype) {
      setFormData({
        eventtype_name: eventtype.name ?? eventtype.eventtype_name ?? "",
        eventtype_description:
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
        ...
        <div className="bg-gradient-to-r from-yellow-500 to-orange-500 px-6 py-4 flex justify-between items-center">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-xl font-semibold text-white">Edit Event Type</h2>
          </div>
          <button
            onClick={handleClose}
